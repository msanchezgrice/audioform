...
export function hostedMcpServices(request: Request): HostedMcpServices {
  ...
  return {
    createHandoff: (input) => run(async () => createHandoff(await authorize("handoffs:write"), { ...input, baseUrl: process.env.TALKFORM_APP_URL?.trim() || new URL(request.url).origin })),
    getHandoff: (id) => run(async () => getHandoff(await authorize("handoffs:read"), id)),
    getResult: (id) => run(async () => getHandoffResult(await authorize("handoffs:read"), id)),
    deleteHandoff: (id) => run(async () => deleteHandoff(await authorize("handoffs:delete"), id)),
  };
}
