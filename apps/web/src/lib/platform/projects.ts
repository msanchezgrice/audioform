...
export function cleanProjectName(input: unknown) {
...
export async function listProjects(clerkUserId: string) {
  const rows = await platformDatabase()<ProjectRow[]>`select id, name, environment, owner_kind, daily_handoff_limit, created_at, updated_at, claimed_at from tf_projects where clerk_user_id = ${clerkUserId} and owner_kind='human' order by created_at desc`;
...
export async function createProject(clerkUserId: string, input: { name: unknown; environment: unknown }) {
  ...
  return platformDatabase().begin(async (tx) => {
    ...
    const [row] = await tx<ProjectRow[]>`insert into tf_projects (clerk_user_id, owner_kind, name, environment) values (${clerkUserId}, 'human', ${name}, ${environment}) returning id, name, environment, owner_kind, daily_handoff_limit, created_at, updated_at, claimed_at`;
...
export async function getProjectDashboard(clerkUserId: string, projectId: string): Promise<ProjectDashboard> {
  ...
  const [projectRows, keyRows, handoffRows, countRows, usageRows] = await Promise.all([
    sql<ProjectRow[]>`select id, name, environment, owner_kind, daily_handoff_limit, created_at, updated_at, claimed_at from tf_projects where id = ${projectId} and clerk_user_id = ${clerkUserId} and owner_kind='human'`,
...
export async function getOwnedProject(clerkUserId: string, projectId: string) {
  const [row] = await platformDatabase()<ProjectRow[]>`select id, name, environment, owner_kind, daily_handoff_limit, created_at, updated_at, claimed_at from tf_projects where id=${projectId} and clerk_user_id=${clerkUserId} and owner_kind='human'`;
