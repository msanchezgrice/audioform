...
import { costIdentityForRequest, hashCostSubject, type CostIdentity } from "./database";

export async function resolveCostIdentity(request: Request): Promise<{ identity: CostIdentity; owner: RequestOwner }> {
  ...
  if (!handoffId && !respondentToken) return { identity: base, owner };
