...
export async function DELETE(request:Request,{params}:{params:Promise<{handoffId:string}>}) { try { const key=await authenticateProjectKey(request,"handoffs:delete"); await consumePlatformRateLimit(request,`handoffs:delete:${key.keyId}`,60); const {handoffId}=await params; return platformJson(await deleteHandoff(key,requireUuid(handoffId),platformEventContext(request,"rest"))); } catch(error){ return platformErrorResponse(error); } }
