...
export async function GET(request:Request,{params}:{params:Promise<{handoffId:string}>}) { try { const key=await authenticateProjectKey(request,"handoffs:read"); await consumePlatformRateLimit(request,`handoffs:result:${key.keyId}`); const {handoffId}=await params; return platformJson(await getHandoffResult(key,requireUuid(handoffId))); } catch(error){ return platformErrorResponse(error); } }
