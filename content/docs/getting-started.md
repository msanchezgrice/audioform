...
4. Send the returned `shareText` (or `respondentUrl`) to the person. The hosted page shows who the interview is from and what it is for when you set `config.branding`. They answer in text, then review and explicitly submit the structured values. Hosted voice becomes available only after the optional human claim described below.
