#!/usr/bin/env python3
"""Create one hosted Talkform handoff and wait a bounded time for its result.

This example uses only the Python standard library. Set TAPK to a Talkform
project API key. The key is sent only in the Authorization header and is never
written to the state file or printed.
"""
...
class TalkformClient:
    def __init__(self, base_url: str, api_key: str, timeout: int = REQUEST_TIMEOUT_SECONDS) -> None:
    ...
    def _redact(self, value: str) -> str:
        return value.replace(self.api_key, "[REDACTED]")[:300]
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not api_key:
        parser.error("Set TAPK to a Talkform project API key. The example does not accept keys on the command line.")
    try:
